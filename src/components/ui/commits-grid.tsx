...
export const CommitsGrid = ({ text, size = "normal" }: { text: string; size?: "small" | "normal" }) => {
  ...
  return (
    <section
      className={cn(
        "bg-card border grid rounded-[6px]",
        isSmall 
          ? "w-auto max-w-none p-0.5 gap-[1px]" 
          : "w-full max-w-xl p-1.5 sm:p-3 gap-0.5 sm:gap-1 rounded-[10px] sm:rounded-[15px]"
      )}
      style={{
